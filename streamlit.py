    ...
    with open('audios.txt', 'r') as f:
        try:
            if l <= 1:
                ...
                with st.sidebar:
                    with col2:
                        select = st.selectbox('Select audio:', ['Recording ' + str(i) for i in range(1, l + 1)])
                        num = int(select[-1]) - 1
            sig = f.readlines()[num]
...
librosa.display.specshow(Xdb, sr=fs, x_axis='time', y_axis='hz', cmap='magma')
...
st.markdown("Here's the audio's Mel spectrogram.")
# Plot Mel Spectrogram
mel_sig = librosa.feature.melspectrogram(y=y, sr=sr)
mspectrogram = np.abs(mel_sig)
power_to_db = librosa.power_to_db(mspectrogram, ref=np.max)
